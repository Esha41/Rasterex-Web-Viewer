...
var foxitViewer = function foxitViewer(zsdivid, divnum, libpath) {
    ...
    this.zoomtopointdirect = function(pagenum, factor, newpos){
     ...
     this.zoomToPoint = function (pagenum, factor, deltaf, mousepoint, offset, center, bIn) {
        if (isNaN(factor) || factor >= foxview.nMaxScale) {
            return;
        }
debugger
        if (foxview.pdfViewer && pagenum == foxview.curpage && factor > 0) {
            try {
                if (foxview.rendering) {
                    return;
                }

                foxview.scale = factor;

                // Get the current page dimensions and scale
                return foxview.pdfViewer.getCurrentPDFDoc().getPageByIndex(pagenum)
                    .then(page => {
                        // Use the mouse coordinates directly as the zoom center point
                        const position = {
                            pageIndex: pagenum,
                            x: mousepoint.mx,  // Use the raw mouse x coordinate
                            y: mousepoint.my   // Use the raw mouse y coordinate
                        };
                        ...
                        return foxview.pdfViewer.zoomAtPosition(foxview.scale, position)
                            .then((result) => {
                                console.log('Zoom Success Result:', result);
                    ...
                    .catch((error) => {
                        ...
                        foxview.rendering = false;
                        foxview.pagestates[pagenum].rendered = true;
                    });
            } catch (error) {
                console.error('Error in zoomToPoint:', error);
                foxview.rendering = false;
                foxview.pagestates[pagenum].rendered = true;
            }
        }
